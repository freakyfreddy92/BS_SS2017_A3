...
void init_data(int length) {
    ...
    for(i = 0; i < length; i++) {
        val = rand() % RNDMOD;
        vmem_write(i, val);			//? hier wird also zunächst auf das shared memory geschrieben bis es voll ist? (hat ja nur 128 Plätze 
    }   /* end for */				//und LENGTH is 550 also wird iwann auch in den Pagefile geschrieben werden müssen? hmmm
    								//die virtuellen Adressen fangen also bei 0 an und gehen bis 550.. aber real gibts nur 128 adresse.. -> umrechnung in
}									//den vmread/write funktionen!! aber wie offset bestimmen? vgl vorlesung vllt?
