...
void init_pagefile(void) {		
    ...
    for(i = 0; i < (VMEM_PAGESIZE * VMEM_NPAGES * sizeof(int)); i++) {  //hier werden also erstmal 1024(=virtueller Speicher) erzeugt die einfach darstellen
        ...
        fwrite(&rndval, 1, 1, pagefile);						//der zu sortierenden Zahlen aus der Applikation werden diese dann teilweise überschrieben mit 
    }															//diesen zu sortierenden Zahlen? Damit man dann quasi die richtigen/zur Sortierung gehörigen
...
void store_page_to_pagefile(int pt_idx, int *frame_start) {
    ...
    int offset = pt_idx * sizeof(int) * VMEM_PAGESIZE;			//??? wie läuft das mit dem Offset?
