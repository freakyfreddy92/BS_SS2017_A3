...
#define PTF_DIRTY       2 //!< store: need to write, entspricht M Bit aus Vorlesung
#define PTF_REF         4       
...
struct pt_struct {
    ...
    int framepage[VMEM_NFRAMES];          //!< Gives for each frame the page stored in this frame.  VOID_IDX indicates an unused frame.A
    								//also wenn man PAGESIZE vergrößert dann verkleinert sich die Größe dieser Arrays (gibt also weniger Seitenrahmen..
    								// eine Zahl aus dem Zahlengenerator verbraucht somit quasi mehr Platz auf dem Hauptspeicher.. man müsste eig. häufiger
};									//Pagefaults haben und somit häufiger ein und auslagern..
...
struct vmem_struct {
    ...
    int data[VMEM_NFRAMES * VMEM_PAGESIZE];  //!< main memory used by virtual memory simulation.. müsste eig. Groeße von PHYSMEMSIZE haben, also 128
};
